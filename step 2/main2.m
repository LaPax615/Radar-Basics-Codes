...
target_1 = [r_max; v_max]*0.1; 
target_2 = [r_max; v_max]*0.2;
target_3 = [r_max; v_max]*0.3;
...
targets = {target_1*scale, target_2*scale, target_3*scale, target_4*scale, target_5*scale, target_6*scale, target_7*scale, target_8*scale, target_9*scale};
...
for k = 1:K
    phi_emitted(k,:) = pi*k*beta*T^2 + pi*beta*t_prime.^2;
end
...
for i = 1:n_targets
    ...
    for k = 1:K
        phi_received_current_current(k,:) = pi*k*beta*T^2 + pi*beta*(t_prime-delay_target_current(t_prime)).^2; %#ok<SAGROW>
    end
    ...
    for k = 1:K
        video_signal_1_current_current(k,:) = kapa * exp(1j*2*pi*f_B*t_prime) .* exp(1j*2*pi*f_D*k*T); %#ok<SAGROW> %video signal x(t)
    end
...
%----Printing----
fprintf('S: amplitude_min = %f, amplitude_max = %f, BWD = %f Hz.\n', min(abs(S)), max(abs(S)), bandwidth_emitted);
fprintf('R: amplitude_min = %f, amplitude_max = %f, BWD = %f Hz.\n\n', min(abs(R)), max(abs(R)), bandwidth_received);
